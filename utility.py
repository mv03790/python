...
def calculate_total_distance(routes, px, py, depot):
    ...
    for route in routes:
        ...
        for i in range(len(route) - 1):
            total_distance += calculate_euclidean_distance(
                px, py, route[i], route[i + 1]
            )
...
def visualise_solution(vrp_sol, px, py, depot, title):
    ...
    for k in range(n_routes):
        ...
        plt.plot(
            [px[depot], px[a_route[0]]], [py[depot], py[a_route[0]]], color=cmap(k)
        )
        plt.plot(
            [px[a_route[-1]], px[depot]], [py[a_route[-1]], py[depot]], color=cmap(k)
        )

        # Draw the route: one by one
        for i in range(0, len(a_route) - 1):
            plt.plot(
                [px[a_route[i]], px[a_route[i + 1]]],
                [py[a_route[i]], py[a_route[i + 1]]],
                color=cmap(k),
            )
        ...
        for i in range(len(px)):
            plt.plot(px[i], py[i])
            plt.text(px[i], py[i], str(i), color="black", fontsize=10, ha="right")
